...
# whisper, turbo 모델이 더 좋은 성능을 보여줌
...
class TelloController:
    ...
    def connect(self):
        ...
        self.stream_thread = threading.Thread(target=self._video_stream)
    ...
    def execute_function(self, function_name: str, parameters: Dict[str, Any] = None):
        ...
        try:
            if function_name == "takeoff":
                ...
                self.tello.take_picture(filename)
...
- "사진 찍어줘" -> {"command": "take_photo"}
- "사진 촬영해" -> {"command": "take_photo"}
...
def main():
    ...
    print("- '사진 찍어줘' - 드론으로 사진을 촬영합니다")
    print("- '사진 촬영해' - 드론으로 사진을 촬영합니다")
